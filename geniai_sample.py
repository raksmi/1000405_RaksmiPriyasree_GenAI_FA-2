...
def load_custom_css():
    """Injects custom CSS for glassmorphism and animations."""
    st.markdown("""
    <style>
    ...
    with tab1:
        ...
        with col_left:
            ...
            with col_s1:
                ...
                selected_season = st.selectbox(get_text('current_season'), season_options)
            ...
            with col_s2:
                ...
                selected_weather = st.selectbox(get_text('day_type'), weather_options)
...
if not advice_text:
    ...
    return
                        ...
                        # Continuing inside main_app() -> tab1
        with col_right:
    ...
    with tab2:
        ...
        if st.button(get_text('send'), key="send_mental_btn", use_container_width=True):
            if user_msg:
                with st.spinner("💭 don't worry..."):
                    try:
                        # Security Note: It's better to use st.secrets["GOOGLE_API_KEY"] here
                        ...
                        # Clean text for TTS (removing markdown-style bolding/italics)
        ...
        # Display Mental Health Response if it exists
